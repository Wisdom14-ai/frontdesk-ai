...
The `WHATSAPP_PLATFORM_*` / `PLATFORM_*` values are optional aliases for platform-level Evolution credentials. The dedicated runner secrets are optional when `CRON_SECRET` is set, because runner routes accept `CRON_SECRET` as a fallback.
